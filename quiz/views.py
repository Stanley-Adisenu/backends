...
from .serializers import DepartmentSerializer,CourseSerializer,QuizSerializer,QuizAttemptSerializer,QuestionSerializer,LevelSerializer,LeaderboardSerializer
...
def leaderboard_view(request):
    # Get all users ordered by their total points
    users = User.objects.all().order_by('-total_points')
    
    leaderboard_data = []
    
    for user in users:
        # Count the number of quiz attempts for each user
        quiz_attempts_count = QuizAttempt.objects.filter(user=user).count()
        
        # Prepare user data
        user_data = {
            'user': UserProfileSerializer(user).data,  # Serialize user data
